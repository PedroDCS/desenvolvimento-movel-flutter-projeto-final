...
class _CadGastoState extends State<CadGasto> {
  ...
  void criaDropDownButtonTipoGasto() async {
    _tgs = await CTipoGastos().getAllList();

    setState(() {
      TipoGasto tgTeste;
      if (!_tgs.isEmpty)
        tgTeste = _tgs.elementAt(0); //_tgSelecionado = _tgs.elementAt(0);
      _tgSelecionado = tgTeste;
      _dropTgs = _tgs.map<DropdownMenuItem<TipoGasto>>((TipoGasto s) {
  ...
  void deletaGasto(int id) async {
    //deleta o gasto
    await CGastos().deletar(id);
  ...
  showAlert(Gasto g) async {
    ...
    TipoGasto tg = await CTipoGastos().get(g.tipo_gasto_id);
    ...
    AlertDialog alerta = AlertDialog(
      ...
      content: tg == null
          ? //faz o teste se o tipo de gasto é nulo, se for mostra um progress
